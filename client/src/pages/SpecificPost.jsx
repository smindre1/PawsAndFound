    ...
    <div>
      <div className="postLayout">
        ...
        <p className="alignText">
          Species/LastSeen/Status: {petData.species || "none"}, {petData.lastseen || "none"}, {petData.type || "none"}
        </p>
      ...
      <div>
        {replies ? (
          replies.map((item) => {
            return (
              <div className="replyLayout" key={replyKey}>
                <p className="alignText">{item.username}</p>
                <p className="alignText">{item.message}</p>
              </div>
            );
          })
        ) : (
      ...
      <form className='replyForm' onSubmit={handleSubmit}>
        ...
        <input className='replyInput' type="text" value={message} onChange={(e) => setMessage(e.target.value)}></input>
        <button className='replyBtn' autofocus type="submit">Add Reply</button>
      </form>
    </div>
  );
};
